	...
	<div class="page-header">
		<h1 class="page-title">
			<div class="icon-and-text">
				<svg-icon icon-class="category" size="2rem"></svg-icon>
	...
	<div class="bg">
		<div class="page-container">
			...
			<div class="left">
				...
				<div class="tag-box">
					<div class="tag-title">
						<svg-icon icon-class="category" color="#70B6FF"></svg-icon>
						分类 ({{ categoryList.length }})
					</div>
					<div class="tag-list">
						<a
							...
							class="tag-option hand-style"
							@click="handleClick({id : tag.id, index : index})"
							v-for="(tag, index) in categoryList"
							:key="index"
						>
							{{ tag.categoryName }} <span class="num">{{ tag.articleCount }}</span>
						</a>
					</div>
				</div>
				<hr class="divider"/>
...
onMounted(async () => {
	...
	queryParams.value.action.categoryId = categoryList.value[0].id;
	getList();
});
...
function handleClick({id, index}: { id: any, index: any }) {
	...
	queryParams.value.action.categoryId = id;
	articleList.value = [];
	getList();
}
</script>
