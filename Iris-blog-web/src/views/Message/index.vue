<template>
	...
	<div class="message-container">
		...
		<div class="message-input">
			<input class="input" v-model="content" @click="show = true" @keyup.enter="send" placeholder="说点什么吧"/>
...
onMounted(async () => {
	messageList.value = await getMessageList();
});
const send = async () => {
	...
	const userAvatar = user.avatar ? user.avatar : blog.blogInfo.touristAvatar;
...
.message-input .input {
	...
	background: rgba(255, 255, 255, 0.2); /* 设置背景色和透明度 */
...
.danmaku-container {
	...
	background: url("https://ik.imagekit.io/irisblog/home.jpg?updatedAt=1723000780641") center no-repeat;
