<template>
	<n-modal class="bg" v-model:show="dialogVisible" preset="dialog" :show-icon="false" transform-origin="center"
		...
		<n-button ref="registerRef" class="mt-11" color="#70B6FF" style="width:100%" :loading="loading"
...
const sendCode = async () => {
	...
	start(60);
	await getCode(registerForm.value.username);
...
const handleRegister = async () => {
	...
	try {
		...
		let loginForm: { password: string; username: string ; captchaCode: string} = {
			username: registerForm.value.username,
			password: registerForm.value.password,
			captchaCode: registerForm.value.code
		}

		const data = await login(<LoginForm>loginForm);
		if (data) {
			setToken(data);
