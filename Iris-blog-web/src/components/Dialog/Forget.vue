<template>
	<n-modal class="bg" v-model:show="dialogVisible" preset="dialog" :show-icon="false" transform-origin="center"
		...
		<n-button class="mt-11" color="#70B6FF" style="width:100%" @click="handleForget" :loading="loading">
			确定
		</n-button>
...
const sendCode = async () => {
	...
	await getCode(forgetForm.value.username)
...
const handleForget = async () => {
	...
	try {
		await updateUserPassword(forgetForm.value);
