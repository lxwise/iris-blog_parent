...
public class AreaUtils {
    ...
    public static Area parseArea(String pathStr) {
        ...
        for (String path : paths) {
            if (area == null) {
                area = LambdaUtil.findFirst(areas.values(), item -> item.getName().equals(path));
            } else {
                area = LambdaUtil.findFirst(area.getChildren(), item -> item.getName().equals(path));
            }
        }
        return area;
    }
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) { // 避免死循环
            ...
            area = area.getParent();
            if (area == null
                    || Arrays.asList(Area.ID_GLOBAL).contains(area.getId())){ // 跳过父节点为中国的情况
                break;
            }
    ...
    public static <T> List<T> getByType(AreaTypeEnum type, Function<Area, T> func) {
        return LambdaUtil.convertList(areas.values(), func, area -> type.getType().equals(area.getType()));
    }
