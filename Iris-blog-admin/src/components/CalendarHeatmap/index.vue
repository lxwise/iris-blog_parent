...
export default /*#__PURE__*/ defineComponent({
  ...
  setup(props) {
    ...
    let tippyInstances: Instance[], tippySingleton: CreateSingletonInstance

    function initTippy() {
      tippyInstances = tippy(
          Array.from(svg.value!.querySelectorAll('.vch__day__square[data-tippy-content]'))
      )
      if (tippySingleton) {
    ...
    function tooltipOptions(day: CalendarItem) {
      if (props.tooltip) {
        if (day.count !== undefined) {
          ...
          return `${day.count} ${props.tooltipUnit} ${lo.value.on} ${lo.value.months[day.date.getMonth()]} ${day.date.getDate()}, ${day.date.getFullYear()}`
        } else if (props.noDataText) {
          return `${props.noDataText}`
        } else if (props.noDataText !== false) {
          return `0 ${props.tooltipUnit} ${lo.value.on} ${lo.value.months[day.date.getMonth()]} ${day.date.getDate()}, ${day.date.getFullYear()}`
        }
      }
      return undefined
    }
