<template>
  ...
  <div class="mb-15px">
    <el-button type="success" plain @click="handleStatus(1)" v-hasPermi="['system:leave:update']">
      <Icon icon="ep:finished" class="mr-5px" />
      批量通过
    </el-button>
    <el-button type="warning" plain @click="handleStatus(2)" v-hasPermi="['system:leave:update']">
      <Icon icon="ep:finished" class="mr-5px" />
      批量驳回
    </el-button>
    <el-button type="danger" plain @click="handleDelete()" v-hasPermi="['system:leave:delete']">
      <Icon icon="ep:finished" class="mr-5px" />
      批量删除
    </el-button>
  </div>
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['system:leave:update']"
          >
          ...
          <el-button
            ...
            v-hasPermi="['system:leave:delete']"
          >
            <Icon icon="ep:delete" />
            删除
          </el-button>
...
import * as LeaveMessageApi from '@/api/message/leave/index'
import { ElTable } from 'element-plus'
import { CommonStatusEnum } from '@/utils/constants'

const createTime = ref([])
/** 用户留言表 列表 */
defineOptions({ name: 'MessageLeave' })
...
/** 行checkbox 选中事件 */
function handleSelectionChange(selection: LeaveMessageApi.LeaveMessageVO[]) {
...
const getList = async () => {
  ...
  try {
    ...
    const data = await LeaveMessageApi.getLeaveMessagePage(queryParams)
...
const handleDelete = async (id?: number) => {
  try {
    ...
    // 发起删除
    await LeaveMessageApi.deleteLeaveMessage(ids)
...
const handleStatus = async (status: number, id?: number) => {
  try {
    ...
    await message.confirm('确认要"' + text + '"留言吗?')

    // 确保在这里ids是一个number数组，然后发起修改状态请求
    await LeaveMessageApi.updateLeaveMessageStatus({ status, ids })
...
const handleExport = async () => {
  try {
    ...
    const data = await LeaveMessageApi.exportLeaveMessageExcel(queryParams)
    download.excel(data, '用户留言表.xls')
  } catch {
  } finally {
