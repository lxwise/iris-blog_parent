    ...
    >
      ...
      <el-form-item label="菜单类型" prop="type">
        <el-radio-group v-model="formData.type">
          <el-radio-button v-for="type in SystemMenuTypeEnum" :key="type" :label="type">
            {{ MenuApi.menuTypeToText(type) }}
          </el-radio-button>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="菜单状态" prop="status">
        <el-radio-group v-model="formData.status">
          <el-radio v-for="status in CommonStatusEnum" :key="status" :label="status">
            {{ status === 1 ? '正常' : '禁用' }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
