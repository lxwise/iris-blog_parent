...
!function(){
  ...
  c.prototype.update = function () {
    ...
    (this.r = n("fnr"), .4 < Math.random() ? (this.x = n("x"), this.y = 0) : (this.x = window.innerWidth, this.y = n("y")),
      this.s = n("s"), this.r = n("r"));
  };
  (SakuraList = function () { this.list = []; }).prototype.push = function (A) { this.list.push(A); };
  SakuraList.prototype.update = function () { for (var A = 0, i = this.list.length; A < i; A++) this.list[A].update(); };
  SakuraList.prototype.draw = function (A) { for (var i = 0, B = this.list.length; i < B; i++) this.list[i].draw(A); };
  SakuraList.prototype.get = function (A) { return this.list[A]; };
  SakuraList.prototype.size = function () { return this.list.length; };
  window.onresize = function () { e.height = window.innerHeight, e.width = window.innerWidth; };
  i.onload = function() {
    let i = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || window.oRequestAnimationFrame;
    e.height = window.innerHeight,
      e.width = window.innerWidth,
      e.setAttribute("class", "canvas_effects " + R),
    ...
    function animate() {
      var A = document.documentElement.classList.contains("night");
      ("all" === R || "day" === R && !A || "night" === R && A) && (t.clearRect(0, 0, e.width, e.height), o.update(), o.draw(t));
      h = i(animate);
    }
